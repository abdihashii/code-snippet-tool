...
- **Can an attacker decrypt with only the API response?**
  ...
  - An attacker has the `encrypted_dek` and all the public parameters (`kdf_salt`, `kdf_parameters`) needed to _attempt_ to derive the KEK. However, they are missing the password.
